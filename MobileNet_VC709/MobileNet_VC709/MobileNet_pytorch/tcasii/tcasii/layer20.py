...
M1_list_layer20 = M1_list_bias_fuse[19]
w_layer20_dw    = weight_quantized_list[19]

print(w_layer20_dw.shape)

Cout = w_layer20_dw.shape[0]

w_20_reshape = w_layer20_dw.reshape((Cout, 9))
w_layer20_dw = w_layer20_dw.reshape((Cout, 9))
...
w20_dw = np.zeros((9,depth,9))

for i in range(0,Cout,1):
    for j in range(9):
        a = int(i/9)
        b = i%9
        w20_dw[b][a][j] = w_layer20_dw[i][j]
                
w20_dw = w20_dw.astype("int8")      

#%%
bin8  = lambda x : ''.join(reversed( [str((x >> i) & 1) for i in range(8)]))

for i in range(9):
    with open("D:\\Spyder Project\\mobilenetv2_demo\\layer1_to_53_M_V1_bias32b\\tcasii\\layer20\\weight_layer20_"+str(i)+".txt","w+") as f:
        f.write("memory_initialization_radix=2;\n") # 数据以2进制格式存储
        f.write("memory_initialization_vector=\n")
        for j in range(depth):
            temp = ""
            for k in range(9):
                temp = bin8(w20_dw[i][j][k]) + temp
            temp = temp + "\n"
            f.write(temp) 

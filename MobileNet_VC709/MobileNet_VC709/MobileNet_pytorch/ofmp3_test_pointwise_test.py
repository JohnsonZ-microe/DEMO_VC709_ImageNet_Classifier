...
from conv_norm_dp_pw import QuantizedConv2D
kernel_size = 1
stride = 1
padding = 0
ofmp_size = 112

out3_np = QuantizedConv2D(input_scale, input_zero, input_quantized, output_scale, output_zero, weight_scale, weight_zero, weight_quantized, bias, kernel_size, stride, padding, ofmp_size, conv_mode = 2)
...
out_list=[]
for i in sample:
    input_batch_sample = np.full(input_qa.shape, i)
    input_batch_sample = torch.FloatTensor(input_batch_sample)
    model_output, features = model_quantized(input_batch_sample)# Forward propagation
    out = np.int(features['features.0.0'].detach().int_repr().numpy()[0].sum())
    print(out)
    out_list.append(out)
    
#%%
import matplotlib.pyplot as plt
plt.scatter(sample,out_list)
...
for i in range(len(out_list)-1,-1,-1):
    if out_list[i] > out_list[i-1]:
        print(i,sample[i])
        break
...
list_rtn, tunning_point_index, length_index = polygon_line_fitting(out_list)
left_tunnig_point = sample[289]
right_tunnig_point = sample[763]
print(left_tunnig_point,right_tunnig_point )

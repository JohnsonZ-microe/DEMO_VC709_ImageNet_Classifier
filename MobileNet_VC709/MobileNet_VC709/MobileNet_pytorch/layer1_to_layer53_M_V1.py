...
filename = "C:\\Users\\10637\\Pictures\\dog.jpg"
filename = "D:\\test\\n01484850\\ILSVRC2012_val_00017194.JPEG"
filename = "D:\\test\\n02128757\\ILSVRC2012_val_00035332.JPEG"
...
model_quantized = torchvision.models.quantization.mobilenet_v2(pretrained=True,
                                                               quantize=True)  # Load a quantized mobilenet_v2 model
...
for i in range(ofmp1.shape[0]):
    for j in range(ofmp1.shape[1]):
        for k in range(ofmp1.shape[2]):
            if (abs(ofmp1[i][j][k] - out1_extract[i][j][k]) <= 3):
                flag[i][j][k] = 1
...
for i in range(ofmp4.shape[0]):
    for j in range(ofmp4.shape[1]):
        for k in range(ofmp4.shape[2]):
            if (abs(ofmp4[i][j][k] - out4_extract[i][j][k]) <= 3):
                flag[i][j][k] = 1

print(flag.sum() / (112 * 112 * 96) * 100, '%')
...
'''
ofmp7, ofmp8, ofmp9 = pw1_dw_pw2_M_cal(input_scale_list, input_zero_list, input_quantized, ofmp_scale_list, ofmp_zero_list,
                                 weight_scale_list, weight_zero_list, weight_quantized_list, bias_list,
                                 kernel_size_list, stride_list, padding_list, ofm_size_list, M0, n, M1)
'''
...
for i in range(ofmp7.shape[0]):
    for j in range(ofmp7.shape[1]):
        for k in range(ofmp7.shape[2]):
            if (abs(ofmp7[i][j][k] - out7_extract[i][j][k]) <= 3):
                flag[i][j][k] = 1

print(flag.sum() / (56 * 56 * 144) * 100, '%')
...
for i in range(ofmp10.shape[0]):
    for j in range(ofmp10.shape[1]):
        for k in range(ofmp10.shape[2]):
            if (abs(ofmp10[i][j][k] - out10_extract[i][j][k]) <= 3):
                flag[i][j][k] = 1
...
for i in range(ofmp11.shape[0]):
    for j in range(ofmp11.shape[1]):
        for k in range(ofmp11.shape[2]):
            if (abs(ofmp11[i][j][k] - out11_extract[i][j][k]) <= 3):
                flag[i][j][k] = 1

print(flag.sum() / (28 * 28 * 144) * 100, '%')
...
for i in range(ofmp12.shape[0]):
    for j in range(ofmp12.shape[1]):
        for k in range(ofmp12.shape[2]):
            if (abs(ofmp12[i][j][k] - out12_extract[i][j][k]) <= 3):
                flag[i][j][k] = 1

print(flag.sum() / (28 * 28 * 32) * 100, '%')
...
for i in range(ofmp13.shape[0]):
    for j in range(ofmp13.shape[1]):
        for k in range(ofmp13.shape[2]):
            if (abs(ofmp13[i][j][k] - out13_extract[i][j][k]) <= 3):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp13)) * 100, '%')
...
for i in range(ofmp14.shape[0]):
    for j in range(ofmp14.shape[1]):
        for k in range(ofmp14.shape[2]):
            if (abs(ofmp14[i][j][k] - out14_extract[i][j][k]) <= 3):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp14)) * 100, '%')
...
for i in range(ofmp15.shape[0]):
    for j in range(ofmp15.shape[1]):
        for k in range(ofmp15.shape[2]):
            if (abs(ofmp15[i][j][k] - out15_extract[i][j][k]) <= 3):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp15)) * 100, '%')
...
for i in range(ofmp15.shape[0]):
    for j in range(ofmp15.shape[1]):
        for k in range(ofmp15.shape[2]):
            if (abs(ofmp15[i][j][k] - out15_sc_extract[i][j][k]) <= 3):
                flag[i][j][k] = 1
...
for i in range(ofmp16.shape[0]):
    for j in range(ofmp16.shape[1]):
        for k in range(ofmp16.shape[2]):
            if (abs(ofmp16[i][j][k] - out16_extract[i][j][k]) <= 3):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp16)) * 100, '%')
...
for i in range(ofmp17.shape[0]):
    for j in range(ofmp17.shape[1]):
        for k in range(ofmp17.shape[2]):
            if (abs(ofmp17[i][j][k] - out17_extract[i][j][k]) <= 3):
                flag[i][j][k] = 1
...
for i in range(ofmp18.shape[0]):
    for j in range(ofmp18.shape[1]):
        for k in range(ofmp18.shape[2]):
            if (abs(ofmp18[i][j][k] - out18_extract[i][j][k]) <= 3):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp18)) * 100, '%')
...
for i in range(ofmp18.shape[0]):
    for j in range(ofmp18.shape[1]):
        for k in range(ofmp18.shape[2]):
            if (abs(ofmp18[i][j][k] - out18_sc_extract[i][j][k]) <= 3):
                flag[i][j][k] = 1
...
for i in range(ofmp19.shape[0]):
    for j in range(ofmp19.shape[1]):
        for k in range(ofmp19.shape[2]):
            if (abs(ofmp19[i][j][k] - out19_extract[i][j][k]) <= 3):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp19)) * 100, '%')
...
for i in range(ofmp20.shape[0]):
    for j in range(ofmp20.shape[1]):
        for k in range(ofmp20.shape[2]):
            if (abs(ofmp20[i][j][k] - out20_extract[i][j][k]) <= 3):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp20)) * 100, '%')
...
for i in range(ofmp21.shape[0]):
    for j in range(ofmp21.shape[1]):
        for k in range(ofmp21.shape[2]):
            if (abs(ofmp21[i][j][k] - out21_extract[i][j][k]) <= 3):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp21)) * 100, '%')
...
for i in range(ofmp22.shape[0]):
    for j in range(ofmp22.shape[1]):
        for k in range(ofmp22.shape[2]):
            if (abs(ofmp22[i][j][k] - out22_extract[i][j][k]) <= 3):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp22)) * 100, '%')
...
for i in range(ofmp23.shape[0]):
    for j in range(ofmp23.shape[1]):
        for k in range(ofmp23.shape[2]):
            if (abs(ofmp23[i][j][k] - out23_extract[i][j][k]) <= 3):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp23)) * 100, '%')
...
for i in range(ofmp24.shape[0]):
    for j in range(ofmp24.shape[1]):
        for k in range(ofmp24.shape[2]):
            if (abs(ofmp24[i][j][k] - out24_extract[i][j][k]) <= 3):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp24)) * 100, '%')
...
for i in range(ofmp24.shape[0]):
    for j in range(ofmp24.shape[1]):
        for k in range(ofmp24.shape[2]):
            if (abs(ofmp24[i][j][k] - out24_sc_extract[i][j][k]) <= 3):
                flag[i][j][k] = 1
...
print(flag.sum() / (get_element_numbers(ofmp25)) * 100, '%')
...
print(flag.sum() / (get_element_numbers(ofmp26)) * 100, '%')
...
print(flag.sum() / (get_element_numbers(ofmp27)) * 100, '%')
...
print(flag.sum() / (get_element_numbers(ofmp28)) * 100, '%')
...
print(flag.sum() / (get_element_numbers(ofmp29)) * 100, '%')
...
print(flag.sum() / (get_element_numbers(ofmp30)) * 100, '%')
#%%
# Layer30 Short Cut
...
print(flag.sum() / (get_element_numbers(ofmp31)) * 100, '%')
...
print(flag.sum() / (get_element_numbers(ofmp32)) * 100, '%')
...
print(flag.sum() / (get_element_numbers(ofmp33)) * 100, '%')
...
print(flag.sum() / (get_element_numbers(ofmp34)) * 100, '%')
...
print(flag.sum() / (get_element_numbers(ofmp35)) * 100, '%')
...
print(flag.sum() / (get_element_numbers(ofmp36)) * 100, '%')
...
with open("C:\\Users\\10637\\Desktop\\new_coe\\MOBILENET_IMAGENET1K_LABEL.txt", "r") as f:

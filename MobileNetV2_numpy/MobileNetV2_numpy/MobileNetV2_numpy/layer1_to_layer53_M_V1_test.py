...
def quantize_tensor(x, num_bits=8):  #num_bits: 量化位数，默认为8位
    ...
    q_x.clamp_(qmin, qmax).floor_() # 将q_x限制在量化范围内，并四舍五入到最近的整数。clamp加上_的含义是直接修改q_x。
    q_x = q_x.floor().byte() #byte是将q_x转换为无符号8位整数类型
...
input_quantized = output_fm_list[2:5]
# ofmp4, ofmp5, ofmp6 = pw1_dw_pw2_M(input_scale_list, input_zero_list, input_quantized, ofmp_scale_list, ofmp_zero_list,
...
ofmp4= QuantizedConv2D_M(input_zero_list[0], input_quantized[0], output_zero_list[0],
                                        weight_quantized_list[0], kernel_size_list[0], stride_list[0], padding_list[0],
                                        ofm_size_list[0], M0[0], n[0], M1[0], conv_mode=2)
ofmp4 = relu(ofmp4, output_zero_list[0])
# print(out1.shape)
ofmp5 = QuantizedConv2D_M(input_zero_list[1], input_quantized[1], output_zero_list[1],
                                        weight_quantized_list[1], kernel_size_list[1], stride_list[1], padding_list[1],
                                        ofm_size_list[1], M0[1], n[1], M1[1], conv_mode=1)
ofmp5 = relu(ofmp5, output_zero_list[1])

ofmp6 = QuantizedConv2D_M(input_zero_list[2], input_quantized[2], ofmp_zero_list[2],
                                        weight_quantized_list[2], kernel_size_list[2], stride_list[2], padding_list[2],
                                        ofm_size_list[2], M0[2], n[2], M1[2], conv_mode=2)
...
for i in range(ofmp4.shape[0]):
    for j in range(ofmp4.shape[1]):
        for k in range(ofmp4.shape[2]):
            if (abs(ofmp4[i][j][k] - out4_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(" Layer 4,5,6 PW_DW_PW Bottleneck with Expansion Layer inference finished ")
print(flag.sum() / (112 * 112 * 96) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes in layer 4")
...
for i in range(ofmp5.shape[0]):
    for j in range(ofmp5.shape[1]):
        for k in range(ofmp5.shape[2]):
            if (abs(ofmp5[i][j][k] - out5_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (56 * 56 * 96) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes in layer 5")
...
for i in range(ofmp6.shape[0]):
    for j in range(ofmp6.shape[1]):
        for k in range(ofmp6.shape[2]):
            if (abs(ofmp6[i][j][k] - out6_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (56 * 56 * 24) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes in layer 6")
# %%
# Layer7 to Layer9
...
# ofmp7, ofmp8, ofmp9 = pw1_dw_pw2_M(input_scale_list, input_zero_list, input_quantized, ofmp_scale_list, ofmp_zero_list,
#                                  weight_scale_list, weight_zero_list, weight_quantized_list, bias_list,
#                                  kernel_size_list, stride_list, padding_list, ofm_size_list, M0, n, M1)

input_quantized = output_fm_list[5:8]
ofmp7= QuantizedConv2D_M(input_zero_list[0], input_quantized[0], output_zero_list[0],
                                        weight_quantized_list[0], kernel_size_list[0], stride_list[0], padding_list[0],
                                        ofm_size_list[0], M0[0], n[0], M1[0], conv_mode=2)
ofmp7 = relu(ofmp7, output_zero_list[0])
# print(out1.shape)
ofmp8 = QuantizedConv2D_M(input_zero_list[1], input_quantized[1], output_zero_list[1],
                                        weight_quantized_list[1], kernel_size_list[1], stride_list[1], padding_list[1],
                                        ofm_size_list[1], M0[1], n[1], M1[1], conv_mode=1)
ofmp8 = relu(ofmp8, output_zero_list[1])

ofmp9= QuantizedConv2D_M(input_zero_list[2], input_quantized[2], ofmp_zero_list[2],
                                        weight_quantized_list[2], kernel_size_list[2], stride_list[2], padding_list[2],
                                        ofm_size_list[2], M0[2], n[2], M1[2], conv_mode=2)
...
for i in range(ofmp7.shape[0]):
    for j in range(ofmp7.shape[1]):
        for k in range(ofmp7.shape[2]):
            if (abs(ofmp7[i][j][k] - out7_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (56 * 56 * 144) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes in layer 7")
...
for i in range(ofmp8.shape[0]):
    for j in range(ofmp8.shape[1]):
        for k in range(ofmp8.shape[2]):
            if (abs(ofmp8[i][j][k] - out8_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (56 * 56 * 144) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes in layer 8")
...
for i in range(ofmp9.shape[0]):
    for j in range(ofmp9.shape[1]):
        for k in range(ofmp9.shape[2]):
            if (abs(ofmp9[i][j][k] - out9_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (56 * 56 * 24) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes in layer 9")
#%%
# Layer9 Short Cut
...
for i in range(ofmp9.shape[0]):
    for j in range(ofmp9.shape[1]):
        for k in range(ofmp9.shape[2]):
            if (abs(ofmp9[i][j][k] - out9_sc_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (56 * 56 * 24) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes after layer 9 shortcut")
# %%
...
ofmp10, ofmp11, ofmp12 = pw1_dw_pw2_M(input_scale_list, input_zero_list, input_quantized, ofmp_scale_list, ofmp_zero_list,
                                 weight_scale_list, weight_zero_list, weight_quantized_list, bias_list,
                                 kernel_size_list, stride_list, padding_list, ofm_size_list, M0, n, M1)


out10_extract = output_fm_list[10]
flag = np.zeros(ofmp10.shape)
for i in range(ofmp10.shape[0]):
    for j in range(ofmp10.shape[1]):
        for k in range(ofmp10.shape[2]):
            if (abs(ofmp10[i][j][k] - out10_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (56 * 56 * 144) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes in layer 10")
out11_extract = output_fm_list[11]
flag = np.zeros(ofmp11.shape)
for i in range(ofmp11.shape[0]):
    for j in range(ofmp11.shape[1]):
        for k in range(ofmp11.shape[2]):
            if (abs(ofmp11[i][j][k] - out11_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (28 * 28 * 144) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes in layer 11")

out12_extract = output_fm_list[12]
flag = np.zeros(ofmp12.shape)
for i in range(ofmp12.shape[0]):
    for j in range(ofmp12.shape[1]):
        for k in range(ofmp12.shape[2]):
            if (abs(ofmp12[i][j][k] - out12_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (28 * 28 * 32) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes in layer 12")
# %%
...
ofmp13, ofmp14, ofmp15 = pw1_dw_pw2_M(input_scale_list, input_zero_list, input_quantized, ofmp_scale_list, ofmp_zero_list,
                                 weight_scale_list, weight_zero_list, weight_quantized_list, bias_list,
                                 kernel_size_list, stride_list, padding_list, ofm_size_list, M0, n, M1)



out13_extract = output_fm_list[13]
flag = np.zeros(ofmp13.shape)
for i in range(ofmp13.shape[0]):
    for j in range(ofmp13.shape[1]):
        for k in range(ofmp13.shape[2]):
            if (abs(ofmp13[i][j][k] - out13_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp13)) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes in layer 13")
out14_extract = output_fm_list[14]
flag = np.zeros(ofmp14.shape)
for i in range(ofmp14.shape[0]):
    for j in range(ofmp14.shape[1]):
        for k in range(ofmp14.shape[2]):
            if (abs(ofmp14[i][j][k] - out14_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp14)) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes in layer 14")
out15_extract = output_fm_list[15]
flag = np.zeros(ofmp15.shape)
for i in range(ofmp15.shape[0]):
    for j in range(ofmp15.shape[1]):
        for k in range(ofmp15.shape[2]):
            if (abs(ofmp15[i][j][k] - out15_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp15)) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes in layer 15")
...
out15_sc_extract = output_fm_list[16]
flag = np.zeros(ofmp15.shape)
for i in range(ofmp15.shape[0]):
    for j in range(ofmp15.shape[1]):
        for k in range(ofmp15.shape[2]):
            if (abs(ofmp15[i][j][k] - out15_sc_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp15)) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes after layer 15 shortcut")
# %%
...
ofmp16, ofmp17, ofmp18 = pw1_dw_pw2_M(input_scale_list, input_zero_list, input_quantized, ofmp_scale_list, ofmp_zero_list,
                                 weight_scale_list, weight_zero_list, weight_quantized_list, bias_list,
                                 kernel_size_list, stride_list, padding_list, ofm_size_list, M0, n, M1)



out16_extract = output_fm_list[17]
flag = np.zeros(ofmp16.shape)
for i in range(ofmp16.shape[0]):
    for j in range(ofmp16.shape[1]):
        for k in range(ofmp16.shape[2]):
            if (abs(ofmp16[i][j][k] - out16_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp16)) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes in layer 16")
out17_extract = output_fm_list[18]
flag = np.zeros(ofmp17.shape)
for i in range(ofmp17.shape[0]):
    for j in range(ofmp17.shape[1]):
        for k in range(ofmp17.shape[2]):
            if (abs(ofmp17[i][j][k] - out17_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp14)) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes in layer 17")
out18_extract = output_fm_list[19]
flag = np.zeros(ofmp18.shape)
for i in range(ofmp18.shape[0]):
    for j in range(ofmp18.shape[1]):
        for k in range(ofmp18.shape[2]):
            if (abs(ofmp18[i][j][k] - out18_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp18)) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes in layer 18")
...
out18_sc_extract = output_fm_list[20]
flag = np.zeros(ofmp18.shape)
for i in range(ofmp18.shape[0]):
    for j in range(ofmp18.shape[1]):
        for k in range(ofmp18.shape[2]):
            if (abs(ofmp18[i][j][k] - out18_sc_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp18)) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes after layer 18 shortcut")
# %%
...
ofmp19, ofmp20, ofmp21 = pw1_dw_pw2_M(input_scale_list, input_zero_list, input_quantized, ofmp_scale_list, ofmp_zero_list,
                                 weight_scale_list, weight_zero_list, weight_quantized_list, bias_list,
                                 kernel_size_list, stride_list, padding_list, ofm_size_list, M0, n, M1)


out19_extract = output_fm_list[21]
flag = np.zeros(ofmp19.shape)
for i in range(ofmp19.shape[0]):
    for j in range(ofmp19.shape[1]):
        for k in range(ofmp19.shape[2]):
            if (abs(ofmp19[i][j][k] - out19_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp19)) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes in layer 19")
out20_extract = output_fm_list[22]
flag = np.zeros(ofmp20.shape)
for i in range(ofmp20.shape[0]):
    for j in range(ofmp20.shape[1]):
        for k in range(ofmp20.shape[2]):
            if (abs(ofmp20[i][j][k] - out20_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp20)) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes in layer 20")
out21_extract = output_fm_list[23]
flag = np.zeros(ofmp21.shape)
for i in range(ofmp21.shape[0]):
    for j in range(ofmp21.shape[1]):
        for k in range(ofmp21.shape[2]):
            if (abs(ofmp21[i][j][k] - out21_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp21)) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes in layer 21")
# %%
...
ofmp22, ofmp23, ofmp24 = pw1_dw_pw2_M(input_scale_list, input_zero_list, input_quantized, ofmp_scale_list, ofmp_zero_list,
                                 weight_scale_list, weight_zero_list, weight_quantized_list, bias_list,
                                 kernel_size_list, stride_list, padding_list, ofm_size_list, M0, n, M1)

out22_extract = output_fm_list[24]
flag = np.zeros(ofmp22.shape)
for i in range(ofmp22.shape[0]):
    for j in range(ofmp22.shape[1]):
        for k in range(ofmp22.shape[2]):
            if (abs(ofmp22[i][j][k] - out22_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp22)) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes in layer 22")
out23_extract = output_fm_list[25]
flag = np.zeros(ofmp23.shape)
for i in range(ofmp23.shape[0]):
    for j in range(ofmp23.shape[1]):
        for k in range(ofmp23.shape[2]):
            if (abs(ofmp23[i][j][k] - out23_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp23)) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes in layer 23")
out24_extract = output_fm_list[26]
flag = np.zeros(ofmp24.shape)
for i in range(ofmp24.shape[0]):
    for j in range(ofmp24.shape[1]):
        for k in range(ofmp24.shape[2]):
            if (abs(ofmp24[i][j][k] - out24_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp24)) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes in layer 24")
#%%
# Layer24 Short Cut
...
out24_sc_extract = output_fm_list[27]
flag = np.zeros(ofmp24.shape)
for i in range(ofmp24.shape[0]):
    for j in range(ofmp24.shape[1]):
        for k in range(ofmp24.shape[2]):
            if (abs(ofmp24[i][j][k] - out24_sc_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp24)) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes after layer 24 shortcut")
# %%
# Layer25 to Layer27
...
ofmp25, ofmp26, ofmp27 = pw1_dw_pw2_M(input_scale_list, input_zero_list, input_quantized, ofmp_scale_list, ofmp_zero_list,
                                 weight_scale_list, weight_zero_list, weight_quantized_list, bias_list,
                                 kernel_size_list, stride_list, padding_list, ofm_size_list, M0, n, M1)


out25_extract = output_fm_list[28]
flag = np.zeros(ofmp25.shape)
for i in range(ofmp25.shape[0]):
    for j in range(ofmp25.shape[1]):
        for k in range(ofmp25.shape[2]):
            if (abs(ofmp25[i][j][k] - out25_extract[i][j][k]) == 0):
                flag[i][j][k] = 1
...
for i in range(ofmp27.shape[0]):
    for j in range(ofmp27.shape[1]):
        for k in range(ofmp27.shape[2]):
            if (abs(ofmp27[i][j][k] - out27_sc_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp27)) * 100, '%'," of values are the same between the pytorch and numpy quantization schemes after layer 27 shortcut")
# %%
...
ofmp28, ofmp29, ofmp30 = pw1_dw_pw2_M(input_scale_list, input_zero_list, input_quantized, ofmp_scale_list, ofmp_zero_list,
                                 weight_scale_list, weight_zero_list, weight_quantized_list, bias_list,
                                 kernel_size_list, stride_list, padding_list, ofm_size_list, M0, n, M1)


out28_extract = output_fm_list[32]
flag = np.zeros(ofmp28.shape)
for i in range(ofmp28.shape[0]):
    for j in range(ofmp28.shape[1]):
        for k in range(ofmp28.shape[2]):
            if (abs(ofmp28[i][j][k] - out28_extract[i][j][k]) == 0):
                flag[i][j][k] = 1
...
for i in range(ofmp30.shape[0]):
    for j in range(ofmp30.shape[1]):
        for k in range(ofmp30.shape[2]):
            if (abs(ofmp30[i][j][k] - out30_sc_extract[i][j][k]) == 0):
                flag[i][j][k] = 1
...
ofmp31, ofmp32, ofmp33 = pw1_dw_pw2_M(input_scale_list, input_zero_list, input_quantized, ofmp_scale_list, ofmp_zero_list,
                                 weight_scale_list, weight_zero_list, weight_quantized_list, bias_list,
                                 kernel_size_list, stride_list, padding_list, ofm_size_list, M0, n, M1)

out31_extract = output_fm_list[36]
flag = np.zeros(ofmp31.shape)
for i in range(ofmp31.shape[0]):
    for j in range(ofmp31.shape[1]):
        for k in range(ofmp31.shape[2]):
            if (abs(ofmp31[i][j][k] - out31_extract[i][j][k]) == 0):
                flag[i][j][k] = 1
...
ofmp34, ofmp35, ofmp36 = pw1_dw_pw2_M(input_scale_list, input_zero_list, input_quantized, ofmp_scale_list, ofmp_zero_list,
                                 weight_scale_list, weight_zero_list, weight_quantized_list, bias_list,
                                 kernel_size_list, stride_list, padding_list, ofm_size_list, M0, n, M1)


out34_extract = output_fm_list[39]
flag = np.zeros(ofmp34.shape)
for i in range(ofmp34.shape[0]):
    for j in range(ofmp34.shape[1]):
        for k in range(ofmp34.shape[2]):
            if (abs(ofmp34[i][j][k] - out34_extract[i][j][k]) == 0):
                flag[i][j][k] = 1
...
for i in range(ofmp36.shape[0]):
    for j in range(ofmp36.shape[1]):
        for k in range(ofmp36.shape[2]):
            if (abs(ofmp36[i][j][k] - out36_sc_extract[i][j][k]) == 0):
                flag[i][j][k] = 1
...
for i in range(ofmp39.shape[0]):
    for j in range(ofmp39.shape[1]):
        for k in range(ofmp39.shape[2]):
            if (abs(ofmp39[i][j][k] - out39_sc_extract[i][j][k]) == 0):
                flag[i][j][k] = 1
...
for i in range(ofmp45.shape[0]):
    for j in range(ofmp45.shape[1]):
        for k in range(ofmp45.shape[2]):
            if (abs(ofmp45[i][j][k] - out45_sc_extract[i][j][k]) == 0):
                flag[i][j][k] = 1
...
for i in range(ofmp48.shape[0]):
    for j in range(ofmp48.shape[1]):
        for k in range(ofmp48.shape[2]):
            if (abs(ofmp48[i][j][k] - out48_sc_extract[i][j][k]) == 0):
                flag[i][j][k] = 1
...
ofmp49, ofmp50, ofmp51 = pw1_dw_pw2_M(input_scale_list, input_zero_list, input_quantized, ofmp_scale_list, ofmp_zero_list,
                                 weight_scale_list, weight_zero_list, weight_quantized_list, bias_list,
                                 kernel_size_list, stride_list, padding_list, ofm_size_list, M0, n, M1)

out49_extract = output_fm_list[58]
flag = np.zeros(ofmp49.shape)
for i in range(ofmp49.shape[0]):
    for j in range(ofmp49.shape[1]):
        for k in range(ofmp49.shape[2]):
            if (abs(ofmp49[i][j][k] - out49_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp49)) * 100, '%',"Layer 49")
out50_extract = output_fm_list[59]
flag = np.zeros(ofmp50.shape)
for i in range(ofmp50.shape[0]):
    for j in range(ofmp50.shape[1]):
        for k in range(ofmp50.shape[2]):
            if (abs(ofmp50[i][j][k] - out50_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

print(flag.sum() / (get_element_numbers(ofmp50)) * 100, '%',"Layer 50")

out51_extract = output_fm_list[60]
flag = np.zeros(ofmp51.shape)
for i in range(ofmp51.shape[0]):
    for j in range(ofmp51.shape[1]):
        for k in range(ofmp51.shape[2]):
            if (abs(ofmp51[i][j][k] - out51_extract[i][j][k]) == 0):
                flag[i][j][k] = 1

ofmp51_2d = ofmp51.reshape((ofmp51.shape[0]*ofmp51.shape[1]),-1)
np.savetxt("ofmp51.csv", ofmp51_2d, delimiter=",")
out51_extract_2d =  out51_extract.reshape(( out51_extract.shape[0]* out51_extract.shape[1]),-1)
np.savetxt("out51_extract.csv", out51_extract_2d, delimiter=",")
flag_2d = flag.reshape((flag.shape[0]*flag.shape[1]),-1)
np.savetxt("Layer51.csv", flag_2d, delimiter=",")
print(flag.sum() / (get_element_numbers(ofmp51)) * 100, '%',"Layer 51")
...
'''
for i in range(1000):
    w_mul_x = np.multiply(weight_quantized[i] - weight_zero, ofmp52_pooling - input_zero).sum()
    output_1000[i] = np.floor(weight_scale*input_scale/output_scale*w_mul_x + output_zero + bias[i]/output_scale)
'''       
#%%

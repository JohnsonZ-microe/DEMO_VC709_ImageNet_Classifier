...
input_quantized = input_zero + input_batch / input_scale
input_quantized = np.round(input_quantized).astype("uint8")
np.save("compare\\input.npy", input_quantized)
with pd.ExcelWriter(f"compare\\input.xlsx") as writer:
...
with pd.ExcelWriter(f"compare\\weight1.xlsx") as writer:
...
with pd.ExcelWriter(f"compare\\numpy2.xlsx") as writer:
    for ch in range(ofmp[2].shape[0]):
        ...
        df.to_excel(writer, sheet_name=f"channel_{ch}", index=False, header=False)
with pd.ExcelWriter(f"compare\\pytorch2.xlsx") as writer:
    for ch in range(out2_extract.shape[0]):
        df = pd.DataFrame(out2_extract[ch])
        df.to_excel(writer, sheet_name=f"channel_{ch}", index=False, header=False)
np.save("compare\\numpy2.npy",ofmp[2])
np.save("compare\\pytorch2.npy",out1_extract)

...
def QuantizedConv2D_M( input_zero, input_quantized, output_zero, weight_quantized, kernel_size, stride, padding, ofm_size, M0, n, M1, conv_mode = 0):# input 3D, weight 4D
    ...
    if(conv_mode == 0):# norm conv
        ...
        for i in range(output.shape[0]):
            for j in range(ofm_size):
                for k in range(ofm_size):
                    ...
                    # output[i][j][k] = ((M0*conv_cal(conv_mode, input_quantized_padding, weight_quantized, kernel_size, stride, i, j, k)).item() + M1[i])*(2**(-n)) + output_zero
                    #将计算结果四舍五入到最近的整数
        ...
        for i in range(output.shape[0]):
            for j in range(output.shape[1]):
                for k in range(output.shape[2]):
                    ...
                    output[i][j][k] = (M0*conv_cal(conv_mode, input_quantized_padding[i], weight_quantized[i][0], kernel_size, stride, i, j, k).item() + M1[i])*(2**(-n)) + output_zero
                    output[i][j][k] = np.round(output[i][j][k])
        ...
        #此处可以直接相减吗？ 数组能直接减去单个值吗？
        #在 numpy 中，支持广播机制（broadcasting），即：
        #如果 input_quantized 是一个多维数组（如 shape 为 [C, H, W]），input_zero 是一个标量（如 int 或 float），
        #那么 input_quantized - input_zero 会自动将 input_zero 广播成和 input_quantized 相同形状，然后逐元素相减。
        #因此，这里可以直接进行相减操作，而不需要使用 np.full 创建一个与 input_quantized 相同形状的数组。
        ...
        for i in range(output.shape[0]):
            for j in range(output.shape[1]):
                for k in range(output.shape[2]):
                    output[i][j][k] = (M0*conv_cal(conv_mode, input_quantized, weight_quantized, kernel_size, stride, i, j, k).item() + M1[i])*(2**(-n)) + output_zero
                    output[i][j][k] = np.round(output[i][j][k])
    return  output
...
def QuantizedConv2D_M( input_zero, input_quantized, output_zero, weight_quantized, kernel_size, stride, padding, ofm_size, M0, n, M1, conv_mode = 0):# input 3D, weight 4D
    ...
    return output
'''
#pw1_dw_pw2_M_cal函数构建一个bottleneck结构的卷积层，其中包含一个pointwise卷积层、一个depthwise卷积层和另一个pointwise卷积层，M0为第一个pointwise卷积层的缩放因子，n为第一个pointwise卷积层的右移位数，M1为第二个pointwise卷积层的缩放因子
#M0具体计算为 2**16*input_scale[0]*weight_scale[0]/output_scale[0]，n为16，M1为2**16*input_scale[1]*weight_scale[1]/output_scale[1]
def pw1_dw_pw2_M_cal(input_scale, input_zero, input_quantized, output_scale, output_zero, weight_scale, weight_zero, weight_quantized, bias, kernel_size, stride, padding, ofm_size, M0, n, M1):
    out1 = QuantizedConv2D_M(input_zero[0], input_quantized, output_zero[0], weight_quantized[0], kernel_size[0], stride[0], padding[0], ofm_size[0], int(2**16*input_scale[0]*weight_scale[0]/output_scale[0]), 16, np.round(bias[0]/output_scale[0]), conv_mode = 2)
    out1 = relu(out1, output_zero[0])
    #print(out1.shape)
    out2 = QuantizedConv2D_M(input_zero[1], out1, output_zero[1], weight_quantized[1], kernel_size[1], stride[1], padding[1], ofm_size[1], int(2**16*input_scale[1]*weight_scale[1]/output_scale[1]), 16, np.round(2**16*bias[1]/output_scale[1]),conv_mode = 1)
    out2 = relu(out2, output_zero[1])
    out3 = QuantizedConv2D_M(input_zero[2], out2, output_zero[2], weight_quantized[2], kernel_size[2], stride[2], padding[2], ofm_size[2], int(2**16*input_scale[2]*weight_scale[2]/output_scale[2]), 16, np.round(2**16*bias[2]/output_scale[2]), conv_mode = 2)
    return out1, out2, out3
#pw1_dw_pw2_M函数与pw1_dw_pw2_M_cal函数类似，但使用了M0、n和M1作为参数，而不是直接计算它们的值。
def pw1_dw_pw2_M(input_scale, input_zero, input_quantized, output_scale, output_zero, weight_scale, weight_zero, weight_quantized, bias, kernel_size, stride, padding, ofm_size, M0, n, M1):
    out1 = QuantizedConv2D_M(input_zero[0], input_quantized, output_zero[0], weight_quantized[0], kernel_size[0], stride[0], padding[0], ofm_size[0], M0[0], n[0], M1[0], conv_mode = 2)
    out1 = relu(out1, output_zero[0])
    #print(out1.shape)
    out2 = QuantizedConv2D_M(input_zero[1], out1, output_zero[1], weight_quantized[1], kernel_size[1], stride[1], padding[1], ofm_size[1], M0[1], n[1], M1[1],conv_mode = 1)
    out2 = relu(out2, output_zero[1])
    out3 = QuantizedConv2D_M(input_zero[2], out2, output_zero[2], weight_quantized[2], kernel_size[2], stride[2], padding[2], ofm_size[2], M0[2], n[2], M1[2], conv_mode = 2)
    return out1, out2, out3

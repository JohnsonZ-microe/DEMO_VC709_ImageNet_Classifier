#本文件用于测试当每层输入与pytorch模型相同时，输出的相似度
import sys
...
# input_quantized = np.clip(np.round(input_batch / input_scale, 0) + input_zero, 0, 255)
input_quantized = input_zero + input_batch / input_scale
input_quantized = np.round(input_quantized).astype("uint8")
np.save("compare_numpy_torchvision\\input.npy", input_quantized)
with pd.ExcelWriter(f"compare_numpy_torchvision\\input.xlsx") as writer:
    for ch in range(input_quantized.shape[0]):
...
# M0_cal = np.round(M0_cal).astype("int32")
M0     = M0_cal

#calculate the M0 and M1 value
M1_save = M1
M1_cal = 2**16*bias/output_scale
# M1_cal = np.round(M1_cal).astype("int32")
M1 = M1_cal
weight_quantized_save = weight_quantized.reshape(32,9,3)
with pd.ExcelWriter(f"compare_numpy_torchvision\\weight1.xlsx") as writer:
    for ch in range(weight_quantized_save.shape[0]):
        df = pd.DataFrame(weight_quantized_save[ch])
        df.to_excel(writer, sheet_name=f"channel_{ch}", index=False, header=False)
...
with pd.ExcelWriter(f"compare_numpy_torchvision\\numpy1.xlsx") as writer:
    for ch in range(ofmp[1].shape[0]):
        df = pd.DataFrame(ofmp[1][ch])
        df.to_excel(writer, sheet_name=f"channel_{ch}", index=False, header=False)
with pd.ExcelWriter(f"compare_numpy_torchvision\\pytorch1.xlsx") as writer:
    for ch in range(out1_extract.shape[0]):
        df = pd.DataFrame(out1_extract[ch])
        df.to_excel(writer, sheet_name=f"channel_{ch}", index=False, header=False)
np.save("compare_numpy_torchvision\\numpy1.npy",ofmp[1])
np.save("compare_numpy_torchvision\\pytorch1.npy",out1_extract)
    ...
    for j in range(ofmp[2].shape[1]):
        for k in range(ofmp[2].shape[2]):
            if (abs(ofmp[2][i][j][k] - out2_extract[i][j][k]) == 0):
                flag[i][j][k] = 1
with pd.ExcelWriter(f"compare_numpy_torchvision\\numpy2.xlsx") as writer:
    for ch in range(ofmp[2].shape[0]):
        df = pd.DataFrame(ofmp[2][ch])
        df.to_excel(writer, sheet_name=f"channel_{ch}", index=False, header=False)
with pd.ExcelWriter(f"compare_numpy_torchvision\\pytorch2.xlsx") as writer:
    for ch in range(out2_extract.shape[0]):
        df = pd.DataFrame(out2_extract[ch])
        df.to_excel(writer, sheet_name=f"channel_{ch}", index=False, header=False)
np.save("compare_numpy_torchvision\\numpy2.npy",ofmp[2])
np.save("compare_numpy_torchvision\\pytorch2.npy",out1_extract)
...
for n, (start_idx, kernel_size_list, stride_list, padding_list,ofm_size_list, has_sc) in enumerate(bottleneck_configs):
    ...
    x = output_fm_list[out_index-1]
    Fx = output_fm_list[out_index+2]
    ...
    for i in range(3):
        input_scale = input_scale_list[i]
        weight_scale = weight_scale_list[i]
        bias = bias_list[i]
        output_scale = ofmp_scale_list[i]
        M0_cal = 2 ** 16 * input_scale * weight_scale / output_scale
        M1_cal = 2 ** 16 * bias / output_scale
        M0.append(M0_cal)
        M1.append(M1_cal)
    ...
    input_quantized = output_fm_list[out_index - 1 : out_index + 2]
    ofmp[start_idx+1] = QuantizedConv2D_M(input_zero_list[0], input_quantized[0], ofmp_zero_list[0], weight_quantized_list[0], kernel_size_list[0], stride_list[0], padding_list[0], ofm_size_list[0], M0[0], n[0], M1[0], conv_mode = 2)
    ofmp[start_idx+1]= relu(ofmp[start_idx+1], output_zero_list[0])
    #print(out1.shape)
    ofmp[start_idx + 2] = QuantizedConv2D_M(input_zero_list[1], input_quantized[1], ofmp_zero_list[1], weight_quantized_list[1], kernel_size_list[1], stride_list[1], padding_list[1], ofm_size_list[1], M0[1], n[1], M1[1],conv_mode = 1)
    ofmp[start_idx + 2] = relu(ofmp[start_idx+2], output_zero_list[1])
    ofmp[start_idx + 3] = QuantizedConv2D_M(input_zero_list[2], input_quantized[2], ofmp_zero_list[2], weight_quantized_list[2], kernel_size_list[2], stride_list[2], padding_list[2], ofm_size_list[2], M0[2], n[2], M1[2], conv_mode = 2)
    #输出相似度结果
...
input_quantized = output_fm_list[60]
...
# ofmp52_pooling[i] = (ofmp52[i].sum() / 49).astype("int32")
    ofmp52_pooling[i] = np.round(output_fm_list[61][i].sum() /49)

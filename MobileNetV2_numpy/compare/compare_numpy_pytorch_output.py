...
def generate_comparison_groups(start_layer, end_layer, shortcut_layers=None):
    ...
    for layer in range(start_layer, end_layer + 1):
        ...
        groups.append({
            "name": f"Layer{layer}",
            "hardware": f"numpy{layer}.npy",
            "software": f"pytorch{layer}.npy"
        })
        ...
        if layer in shortcut_layers:
            groups.append({
                ...
                "hardware": f"numpy{layer}_shortcut.npy",
                "software": f"pytorch{layer}_shortcut.npy"
            })
...
if __name__ == "__main__":
    ...
    start_layer = 1  # 起始层数
    end_layer = 2  # 结束层数
    shortcut_layers = []  # 包含shortcut的层列表，如 [9, 15, 18]，无则设为[]

module image_uart_ascii_decoder(
input              clk              ,
input              rst_n            ,
input              recv_asc_vld     ,
input      [7:0]   recv_ascii_data  ,
output reg [14:0]  bram_addr_start  ,
output reg         bram_addr_vld    
);

reg              cnt_recv_asc_vld ;
reg              recv_done        ;
reg   [7:0]      recv_data        ;
reg   [3:0]      recv_data_hex    ;

wire recv_asc_done;
reg  recv_asc_vld_r1,recv_asc_vld_r2;

always@(*)begin
	recv_data_hex <= 4'h0;
	case(recv_ascii_data)
		8'h30:
			recv_data_hex <= 4'h0;
		8'h31:
			recv_data_hex <= 4'h1;
...
assign recv_asc_done = (~recv_asc_vld_r2) & recv_asc_vld_r1;
always@(posedge clk or negedge rst_n)begin
	if(!rst_n)begin
		...
		recv_asc_vld_r1 <= recv_asc_vld;
		recv_asc_vld_r2 <= recv_asc_vld_r1;
	end
end
...
always@(posedge clk or negedge rst_n)begin
	if(!rst_n)
		...
		if(recv_asc_done && (cnt_recv_asc_vld == 1'b0))
			recv_data[7:4] <= recv_data_hex;
		else if(recv_asc_done && (cnt_recv_asc_vld == 1'b1))
			recv_data[3:0] <= recv_data_hex;
	end
end
...
always@(posedge clk or negedge rst_n)begin
	if(!rst_n)
		...
		if(recv_asc_done && (cnt_recv_asc_vld == 1'b1))
			recv_done <= 1'b1;
		else
			recv_done <= 1'b0;
	end
...
always@(posedge clk or negedge rst_n)begin
	if(!rst_n)
		cnt_recv_asc_vld <= 1'b0;
	else if(recv_asc_done)begin
		if(cnt_recv_asc_vld == 1'b1)
			cnt_recv_asc_vld <= 1'b0;
		else
			cnt_recv_asc_vld <= cnt_recv_asc_vld + 1'b1;
	end
end

module counter_dw#
  ...
  (
    ...
    output reg shortcut_addr_sel,//用于选择bram此时读出的是x的地址还是fx的地址
  ...
  begin
    ...
    begin
      ...
      begin
        if((Ifmap_hori_base_cnt == 0)&&(Ifmap_vert_base_cnt == 0))
          Ifmap_hori_base_cnt <= 1;
        else if (read_h_block_idx == row_block_num -1)
          Ifmap_hori_base_cnt <= 1;
        else
          Ifmap_hori_base_cnt <= Ifmap_hori_base_cnt + 3;
      end
  ...
  begin
    ...
    begin
      ...
      begin
        if((Ifmap_hori_base_cnt == 0)&&(Ifmap_vert_base_cnt == 0))
          Ifmap_vert_base_cnt <= 1;
        else if (read_v_block_idx == row_block_num -1 && read_h_block_idx == row_block_num -1)
          Ifmap_vert_base_cnt <= 1;
        else if (read_h_block_idx == row_block_num -1)
          Ifmap_vert_base_cnt <= Ifmap_vert_base_cnt + 3;
        else
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if(read_h_block_idx == row_block_num -1 && read_v_block_idx == row_block_num -1)
          Channel_cnt <= Channel_cnt + 1;
        else
          Channel_cnt <= Channel_cnt;
      end
  ...
  reg avgpool_chan_done_reg1,avgpool_chan_done_reg2,avgpool_chan_done_reg3;
  reg avgpool_chan_done_reg4,avgpool_chan_done_reg5,avgpool_chan_done_reg6;
  reg avgpool_chan_done_reg7;
  reg avgpool_chan_start_reg1,avgpool_chan_start_reg2,avgpool_chan_start_reg3;
  reg avgpool_chan_start_reg4,avgpool_chan_start_reg5,avgpool_chan_start_reg6;
  reg avgpool_chan_start_reg7;
  assign avgpool_chan_done_no_sync = (read_h_block_idx == row_block_num -1 && read_v_block_idx == row_block_num -1);
  assign avgpool_chan_start_no_sync = (read_h_block_idx == 1 && read_v_block_idx == 1 && Channel_cnt == 0);
  always @(posedge clk)begin
    if(!rst_n) begin
      avgpool_chan_done_reg1 <= 0;
      avgpool_chan_done_reg2 <= 0;
      avgpool_chan_done_reg3 <= 0;
      avgpool_chan_done_reg4 <= 0;
      avgpool_chan_done_reg5 <= 0;
      avgpool_chan_done_reg6 <= 0;
      avgpool_chan_done_reg7 <= 0;
      avgpool_chan_done <= 0;
      avgpool_chan_start_reg1 <= 0;
      avgpool_chan_start_reg2 <= 0;
      avgpool_chan_start <= 0;
    end
    else begin
      avgpool_chan_done_reg1 <= avgpool_chan_done_no_sync;
      avgpool_chan_done_reg2 <= avgpool_chan_done_reg1;
      avgpool_chan_done_reg3 <= avgpool_chan_done_reg2;
      avgpool_chan_done_reg4 <= avgpool_chan_done_reg3;
      avgpool_chan_done_reg5 <= avgpool_chan_done_reg4;
      avgpool_chan_done_reg6 <= avgpool_chan_done_reg5;
      avgpool_chan_done_reg7 <= avgpool_chan_done_reg6;
      avgpool_chan_done <= avgpool_chan_done_reg7;
      avgpool_chan_start_reg1 <= avgpool_chan_start_no_sync;
      avgpool_chan_start_reg2 <= avgpool_chan_start_reg1;
      avgpool_chan_start <= avgpool_chan_start_reg2;
    end
  end

  reg [1:0] last_block_signal, last_block_signal_reg1, last_block_signal_reg2;
  always @(posedge clk)begin
    if(!rst_n) begin
      last_block_signal <= 0;
      last_block_signal_reg1 <= 0;
      last_block_signal_reg2 <= 0;
    end
    else begin
      last_block_signal <= {read_v_block_idx == row_block_num - 1, read_h_block_idx == row_block_num - 1};
      last_block_signal_reg1 <= last_block_signal;
      last_block_signal_reg2 <= last_block_signal_reg1;
    end
  end
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        else if ((write_h_block_idx == row_block_num - 1)  && (Write_Channel_cnt == ofmap_channel - 1) && write_valid)
          Ofmap_hori_base_cnt<=1;
        else if((Write_Channel_cnt == ofmap_channel - 1) && write_valid)
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        else if ((write_h_block_idx == row_block_num - 1)  && (write_v_block_idx == row_block_num -1)&& (Write_Channel_cnt == ofmap_channel - 1) && write_valid)
          Ofmap_vert_base_cnt<=1;
        else if((write_h_block_idx == row_block_num -1 ) && (Write_Channel_cnt == ofmap_channel - 1) && write_valid)
      ...
      begin
        if(Ofmap_vert_base_cnt == ofmap_size - 1 && Ofmap_hori_base_cnt == ofmap_size - 1 && Write_Channel_cnt == ofmap_channel - 1 && write_valid)
          Ofmap_vert_base_cnt<=0;
        else if(Ofmap_hori_base_cnt == ofmap_size - 1 && Write_Channel_cnt == ofmap_channel - 1 && write_valid)
  ...
  begin
    ...
    begin
      ...
      padding_arbiter <= padding_delay_3;
    end
  end
  ...
  begin
    case(Mode)
      ...
      3'd5: //avgpool
      if(last_block_signal_reg2 == 2'b11)begin
        Array_Mac0_ifmap0 <= Window00_in[ 1 * FM_SIZE_WIDTH -1 : 0 * FM_SIZE_WIDTH];
        Array_Mac0_ifmap1 <= 0;
        Array_Mac0_ifmap2 <= 0;
        Array_Mac0_ifmap3 <= 0;
      ...
      else if (last_block_signal_reg2 == 2'b01) begin
        Array_Mac0_ifmap0 <= Window00_in[ 1 * FM_SIZE_WIDTH -1 : 0 * FM_SIZE_WIDTH];
        Array_Mac0_ifmap1 <= 0;
        Array_Mac0_ifmap2 <= 0;
        Array_Mac0_ifmap3 <= Window10_in[ 1 * FM_SIZE_WIDTH -1 : 0 * FM_SIZE_WIDTH];
      ...
      else if (last_block_signal_reg2 == 2'b10) begin
        Array_Mac0_ifmap0 <= Window00_in[ 1 * FM_SIZE_WIDTH -1 : 0 * FM_SIZE_WIDTH];
        Array_Mac0_ifmap1 <= Window01_in[ 1 * FM_SIZE_WIDTH -1 : 0 * FM_SIZE_WIDTH];
        Array_Mac0_ifmap2 <= Window02_in[ 1 * FM_SIZE_WIDTH -1 : 0 * FM_SIZE_WIDTH];
        Array_Mac0_ifmap3 <= 0;
  ...
  always@(*)
  begin
    if((Ofmap_vert_base_cnt == ofmap_size - 1 && Ofmap_hori_base_cnt == ofmap_size - 1 && Write_Channel_cnt == ofmap_channel - 1 && write_valid)&&(Mode != 3'd4)&&(Mode != 3'd5))
      complete <= 1'b1;
    else if ((Mode == 3'd4)&&(write_h_block_idx == row_block_num -1)&&(write_v_block_idx == row_block_num -1)&&(Write_Channel_cnt == ofmap_channel - 1)&&(write_valid == 1'b1))
      complete <= 1'b1;
  ...
  begin
    ...
    begin
      tlast_pw_delay1 <= tlast_pw_no_sync;
      tlast_pw <= tlast_pw_delay1;
    end
  end
  always@(posedge clk)
  begin

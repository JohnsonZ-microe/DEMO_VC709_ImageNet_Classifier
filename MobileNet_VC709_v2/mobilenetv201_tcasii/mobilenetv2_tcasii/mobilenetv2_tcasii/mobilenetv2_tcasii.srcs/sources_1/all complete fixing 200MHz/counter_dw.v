module counter_dw#
  ...
  always @(posedge clk) begin
    if(!rst_n) shortcut_valid <= 0;
    else shortcut_valid <= {shortcut_valid[29:0], shortcut_addr_sel};
  end
  ...
  always @(posedge clk) begin
    if(!rst_n) kernel_p_nosync <= 0;
    else kernel_p_nosync <= Kernel_cnt;
  end
  ...
  always @( *) begin
    case(Mode_reg[5])
      3'd3:write_valid_mux = pw_valid;
      3'd4:write_valid_mux = shortcut_valid[6];
      default:write_valid_mux = write_valid;
    endcase
  end
  ...
  begin
    case(Mode_reg[0])
      ...
      3'd5: //avgpool
      if(last_block_signal_reg[7] == 2'b11)begin
        Array_Mac0_ifmap0 <= Window00_in[ 1 * FM_SIZE_WIDTH -1 : 0 * FM_SIZE_WIDTH];
        Array_Mac0_ifmap1 <= 0;
        Array_Mac0_ifmap2 <= 0;
        Array_Mac0_ifmap3 <= 0;
      ...
      else if (last_block_signal_reg[7] == 2'b01) begin
        Array_Mac0_ifmap0 <= Window00_in[ 1 * FM_SIZE_WIDTH -1 : 0 * FM_SIZE_WIDTH];
        Array_Mac0_ifmap1 <= 0;
        Array_Mac0_ifmap2 <= 0;
        Array_Mac0_ifmap3 <= Window10_in[ 1 * FM_SIZE_WIDTH -1 : 0 * FM_SIZE_WIDTH];
      ...
      else if (last_block_signal_reg[7] == 2'b10) begin
        Array_Mac0_ifmap0 <= Window00_in[ 1 * FM_SIZE_WIDTH -1 : 0 * FM_SIZE_WIDTH];
        Array_Mac0_ifmap1 <= Window01_in[ 1 * FM_SIZE_WIDTH -1 : 0 * FM_SIZE_WIDTH];
        Array_Mac0_ifmap2 <= Window02_in[ 1 * FM_SIZE_WIDTH -1 : 0 * FM_SIZE_WIDTH];
        Array_Mac0_ifmap3 <= 0;
  ...
  always @(posedge clk) begin
    if(!rst_n) begin
      ...
      if ((Mode_reg[0] == 3'd0)&&(Mode == 3'd3)) begin
        read_last <= 0;
      end
      else if((Mode==3'd3)&& (Channel_cnt==ifmap_channel-1) &&(Kernel_cnt == kernel_num-1) && (Ifmap_hori_base_cnt == ofmap_size-1) && (Ifmap_vert_base_cnt == ofmap_size -1)) begin
        read_last <= 1;
      end
      else begin
        read_last <= read_last;
  ...
  always @(posedge clk) begin
    if(!rst_n) begin
      ...
      if (read_last == 0) begin
        stop_tlast_pw <= 0;
      end
      else if (read_last && tlast_pw_no_sync) begin
        stop_tlast_pw <= 1;
      end
      else begin
  ...
  always @(*) begin
    case(Mode)
      3'd1: tlast_no_sync = (bram_st == CALCULATE);
      3'd2: tlast_no_sync = (bram_st == CALCULATE);
      3'd3: tlast_no_sync = (Channel_cnt==ifmap_channel-1);
      default: tlast_no_sync = 0;
    endcase
  end
  ...
  always @( *) begin
    case(Mode)
      3'd3:tlast_pw_no_sync = stop_tlast_pw ? 1'b0:(Channel_cnt == 1);
      default:tlast_pw_no_sync = 1'b0;
    endcase
  end
  ...
  always @( *) begin
    case(Mode)
      3'd1: tlast = tlast_sync[4];
      3'd2: tlast = tlast_sync[4];
      3'd3: tlast = tlast_sync[7];
      default: tlast = tlast_sync[4];
    endcase
  end

(*use_dsp = "yes"*)(* keep_hierarchy = "yes" *)module MAC(
    input     				     clk     ,
    input     				     rst_n   ,
    input       	             tlast   ,
    input                        acc_mode,
...
always @(posedge clk) begin
	if(!rst_n) begin
		...
		psum_out_reg <= acc_mode? acc_reg:data*weight;//acc_mode = (mode==2'd3);如果是pw，输出累加结果，否则输出当前的乘积结果
	end
end
assign psum_out = psum_out_reg;//acc_mode = (mode==2'd3);如果是pw，输出累加结果，否则输出当前的乘积结果
endmodule

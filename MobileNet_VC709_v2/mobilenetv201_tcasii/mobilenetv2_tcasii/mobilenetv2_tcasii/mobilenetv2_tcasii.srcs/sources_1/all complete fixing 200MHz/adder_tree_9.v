...
wire      add;
reg       tlast_r1,tlast_r2,tlast_r3,tlast_r4,tlast_r5;
...
always@(posedge clk or negedge rst_n)
	if(!rst_n)begin
		...
		tlast_r1 <= first_tlast ? 1'b0: tlast;
		tlast_r2 <= tlast_r1;
		tlast_r3 <= tlast_r2;
		tlast_r4 <= tlast_r3;
		tlast_r5 <= tlast_r4;
...
assign sum_vld = (acc_mode==1) ? tlast_r5:tlast_r4;
assign pw_valid = tlast_r3;
assign add = (acc_mode==1)? tlast_r1: 1'b1;//acc_mode = (mode==2'd3);如果是pw模式，tlast表示最后一个channel的计算完成，否则始终为1

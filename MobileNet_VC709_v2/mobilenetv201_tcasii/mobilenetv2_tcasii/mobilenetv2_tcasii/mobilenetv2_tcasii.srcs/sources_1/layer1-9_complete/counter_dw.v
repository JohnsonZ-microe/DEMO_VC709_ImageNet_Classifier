module counter_dw#
  ...
  assign Mode = General_config[1:0];
  wire [7:0] ifmap_size;
  assign ifmap_size = (Mode==2'd1)? 8'd224: ifmap_size_no_first_layer;
  ...
  begin
    ...
    begin
      ...
      if ((Ifmap_hori_base_cnt == ofmap_size-1) && (Kernel_cnt == kernel_num-1)&&(Mode != 2'd3))
        Ifmap_hori_base_cnt <= 0;
        ...
        else Ifmap_hori_base_cnt <= Ifmap_hori_base_cnt;
      end
  ...
  begin
    ...
    begin
      ...
      if ((Ifmap_vert_base_cnt == ofmap_size-1)&&(Ifmap_hori_base_cnt == ofmap_size-1) && (Kernel_cnt==kernel_num-1)&&(Mode != 2'd3))
        Ifmap_vert_base_cnt <= 0;
      else if(Mode == 2'd3)begin
  ...
  begin
    ...
    begin
      if ((Kernel_cnt == kernel_num-1)&&(Mode != 2'd3))
        Kernel_cnt <= 0;
      else if(Mode == 2'd3)begin
      ...
      else if ((Channel_cnt==ifmap_channel-1)&&(Mode != 2'd2))
        Kernel_cnt <= Kernel_cnt + 1;
      else if (Mode == 2'd2) //Depthwise conv
        Kernel_cnt <= Kernel_cnt + 1;
      else
  ...
  reg tlast_pw_delay1;
  assign tlast_no_sync = (Mode==2'd3)?(Channel_cnt==ifmap_channel-1):(bram_st == CALCULATE);
